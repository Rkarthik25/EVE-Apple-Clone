...
import {Box, Button,useToast, Flex, FormControl, FormHelperText, FormLabel, Heading, Input, Link, Select, Text} from "@chakra-ui/react";
import { useDispatch, useSelector } from 'react-redux';
import { signupRequest, signupSuccess, signupFailure } from '../../redux/authReducer/action';
  ...
  return (
    <Box  ml={"35%"} mr={"35%"} w="30%" mt={5}  > 
       ...
       <Heading>Create Your EVE ID</Heading>
           <Text>One EVE ID is all you need to access all Apple services.
Already have an EVE ID? <span><Link color={"blue"} >Find it here</Link></span></Text>
  ...
  <Box mt={3}>
    <Input type ="date" placeholder="Birthday" name='Birthday' value={Dob} onChange={(e)=> setDob(e.target.value)} />
  </Box>
...
<hr style={{margin: "20px 0px 20px 0px", width:"100%" }} />
  ...
  <Input mt={3} type="password" placeholder='Password' value={password} onChange={(e)=> setPassword(e.target.value)}  />
  {/* <Input mt={3} type="password" placeholder='Confirm password'  /> */}
    {/* </Box> */}
    <hr style={{margin: "20px 0px 20px 0px", width:"100%" }} />
   <Input type={"number"} placeholder="Phone number" value={phone} onChange={(e)=> setPhone(e.target.value)}  />
   <Box mt="20px">
    <Button backgroundColor={"#42a1ec"} onClick={handleClick} color="white" border={"1px solid black"} type="Submit" >Continue</Button>
   </Box>
</FormControl>
